...
co(function*() {
    ...
    for (let repo of repositories) {
        ...
        let statuses = builds.filter(b => b.status != 0).sort((l, r) => { return r.id - l.id }).reduce((acc, build) => {
            if (!acc.has(build['dockertag_name'])) {
                acc.set(build['dockertag_name'], build.status);
            }
            return acc;
        }, new Map());
        success = success && (Array.from(statuses.values()).reduce((acc, val) => Math.min(acc, val), 1) > 0);
        statuses.forEach((status, tag) => {
            let label = status > 0 ?  `\u001b[32mOK\u001b[0m` : `\u001b[31mNG\u001b[0m`;
